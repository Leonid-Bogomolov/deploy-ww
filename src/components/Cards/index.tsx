    ...
    return <div className={style.container}>
        ...
        {updateBeans && updateBeans.map((/deploy-ww/bean)=>{   // updateBeans - обновлённые текущие данные; используем условный рендеринг, т.к. updateBeans может быть null;
         //                                             т.е. если существует тогда выполняем map
          console.log(/deploy-ww/bean)
            return <Card data = {/deploy-ww/bean} key={/deploy-ww/bean.beanId}/> // вместе с данными data необходимо передать ещё уникальный атрибут карточки(key), что бы можно было их сортировать, фильтровать
